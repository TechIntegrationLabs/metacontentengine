...
import { LayoutDashboard, PenTool, FileText, Users, Settings as SettingsIcon, BarChart3, Palmtree, LogOut } from 'lucide-react';
...
const STORAGE_KEY = 'pcc_brand_profile';
...
// Loading spinner component - PCC teal brand color
function LoadingSpinner() {
  return (
    <div className="min-h-screen bg-void-950 flex items-center justify-center">
      <div className="w-12 h-12 border-4 border-pcc-teal/30 border-t-pcc-teal rounded-full animate-spin"></div>
    </div>
  );
}
...
function AuthenticatedApp({ onSignOut }: { onSignOut: () => Promise<void> }) {
  ...
  return (
    <div className="min-h-screen bg-void-950 text-white flex overflow-hidden font-sans">
      ...
      >
        {/* PCC Logo with palm tree icon and teal/gold gradient */}
        <div className="p-6 flex items-center space-x-3 mb-6">
          <div className="w-8 h-8 min-w-[32px] rounded-lg bg-gradient-to-br from-pcc-teal to-pcc-gold flex items-center justify-center shadow-lg shadow-pcc-teal/20">
            <Palmtree className="w-5 h-5 text-white" />
          </div>
          {!isSidebarCollapsed && (
            <span className="text-xl font-display font-bold tracking-tight whitespace-nowrap">
              {tenant?.appName || 'PCC Content'}
            </span>
          )}
        </div>
            ...
            return (
              <button
                ...
                className={[
                  ...
                  active
                    ? 'bg-gradient-to-r from-pcc-teal/10 to-transparent text-pcc-teal border-l-2 border-pcc-teal'
                    : 'text-slate-500 hover:text-slate-200 hover:bg-white/5'
                ].join(' ')}
              >
                <Icon className={['w-5 h-5', active ? 'text-pcc-teal' : ''].join(' ')} />
                {!isSidebarCollapsed && (
        ...
        <div className="p-4 border-t border-white/5 bg-void-900/50">
          <div className={['flex items-center', isSidebarCollapsed ? 'justify-center' : 'space-x-3'].join(' ')}>
            {/* PCC user avatar with teal to gold gradient */}
            <div className="w-8 h-8 min-w-[32px] rounded-full bg-gradient-to-tr from-pcc-teal to-pcc-gold flex items-center justify-center text-xs font-bold">
              {initials}
            </div>
            {!isSidebarCollapsed && (
              <>
      ...
      <main className={['flex-1 overflow-y-auto relative scroll-smooth transition-all duration-500', isSidebarCollapsed ? 'ml-20' : 'ml-64'].join(' ')}>
        ...
        {/* PCC brand background effects - teal and coral */}
        <div className="fixed top-0 right-0 -z-10 w-[800px] h-[800px] bg-pcc-teal/10 blur-[150px] rounded-full pointer-events-none" />
        <div className="fixed bottom-0 left-0 -z-10 w-[600px] h-[600px] bg-pcc-coral/5 blur-[150px] rounded-full pointer-events-none" />
      </main>
    </div>
