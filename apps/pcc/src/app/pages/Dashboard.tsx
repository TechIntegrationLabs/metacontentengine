...
// Helper to get status color - using PCC brand colors
function getStatusColor(status: string): string {
  switch (status) {
    case 'published': return 'emerald';
    case 'humanizing': return 'amber';
    case 'drafting': return 'teal'; // PCC teal
    case 'review': return 'gold'; // PCC gold
    case 'ready': return 'cyan';
    case 'scheduled': return 'coral'; // PCC coral
  ...
  return (
    <div className="max-w-[1400px] mx-auto space-y-6 animate-slide-up pb-10">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <div className="h-[300px] w-full">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={chartData}>
                ...
                <Tooltip contentStyle={{ backgroundColor: '#1a1512', borderColor: '#334155', borderRadius: '12px' }} />
                <Area type="monotone" dataKey="articles" stroke="#2d5a47" strokeWidth={3} fill="url(#colorArticles)" />
              </AreaChart>
            </ResponsiveContainer>
          </div>
        </div>

        {/* Stats cards with PCC brand colors */}
        <div className="glass-card rounded-2xl p-6">
          <div className="flex items-center space-x-2 mb-2">
            <div className="p-1.5 rounded-lg bg-pcc-teal/10 border border-pcc-teal/20">
        ...
        <div className="glass-card rounded-2xl p-6">
          <div className="flex items-center space-x-2 mb-2">
            <div className="p-1.5 rounded-lg bg-pcc-coral/10 border border-pcc-coral/20">
              <Zap className="w-4 h-4 text-pcc-coral" />
            </div>
              ...
              recentActivity.map((item) => (
                <div key={item.id} className="flex items-start space-x-3 p-2 hover:bg-white/5 rounded-lg cursor-pointer">
                  <div className={['mt-1.5 w-2 h-2 rounded-full',
                    ...
                    item.color === 'teal' ? 'bg-pcc-teal' :
                    item.color === 'gold' ? 'bg-pcc-gold' :
                    item.color === 'cyan' ? 'bg-cyan-500' :
                    item.color === 'coral' ? 'bg-pcc-coral' : 'bg-slate-500'
                  ].join(' ')} />
                  <div className="flex-1">
                    ...
                    <div className="flex justify-between mt-1">
                      <span className={['text-[10px] uppercase',
                        ...
                        item.color === 'teal' ? 'text-pcc-teal' :
                        item.color === 'gold' ? 'text-pcc-gold' :
                        item.color === 'cyan' ? 'text-cyan-400' :
                        item.color === 'coral' ? 'text-pcc-coral' : 'text-slate-400'
        ...
        <div className="glass-card rounded-2xl p-6">
          <div className="flex items-center space-x-2 mb-2">
            <div className="p-1.5 rounded-lg bg-pcc-gold/10 border border-pcc-gold/20">
              <Users className="w-4 h-4 text-pcc-gold" />
            </div>
