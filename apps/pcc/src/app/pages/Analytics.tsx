...
const contentPerformance = [
  { category: 'Culture', articles: 24, views: 45000 },
  { category: 'Events', articles: 18, views: 32000 },
  { category: 'Travel', articles: 12, views: 28000 },
  { category: 'History', articles: 8, views: 15000 },
  { category: 'Food', articles: 5, views: 8000 },
];

const topArticles = [
  { title: 'Complete Guide to Polynesian Luau Experience', views: 12453, trend: 15 },
  { title: 'Traditional Hawaiian Crafts and Their Meanings', views: 8921, trend: 23 },
  { title: 'Understanding Polynesian Navigation: Wayfinding', views: 7234, trend: -5 },
  { title: 'Best Time to Visit the Polynesian Cultural Center', views: 6102, trend: 8 },
  { title: 'Exploring the Islands: A Cultural Journey', views: 5234, trend: 12 },
];

// PCC brand colors for charts
const sourceData = [
  { name: 'Organic Search', value: 45, color: '#2d5a47' }, // PCC teal
  { name: 'Direct', value: 25, color: '#c9a227' }, // PCC gold
  { name: 'Social Media', value: 18, color: '#e8734a' }, // PCC coral
  { name: 'Referral', value: 12, color: '#10b981' }, // emerald
];
  ...
  return (
    <div className="space-y-6">
          ...
          {['7d', '14d', '30d', '90d'].map((range) => (
            <button
              ...
              className={[
                ...
                timeRange === range
                  ? 'bg-pcc-teal/20 text-pcc-teal border border-pcc-teal/30'
          ...
          return (
            <div key={i} className="bg-void-900/50 rounded-xl border border-white/5 p-5">
              <div className="flex items-center justify-between mb-3">
                <div className="p-2 rounded-lg bg-pcc-teal/10">
                  <Icon className="w-5 h-5 text-pcc-teal" />
                </div>
                <div className={[
      ...
      <div className="grid grid-cols-3 gap-6">
        {/* Traffic Chart - using PCC teal and gold */}
        <div className="col-span-2 bg-void-900/50 rounded-2xl border border-white/5 p-6">
          <div className="flex items-center justify-between mb-6">
            ...
            <div className="flex items-center space-x-4 text-sm">
              <div className="flex items-center space-x-2">
                <div className="w-3 h-3 rounded-full bg-pcc-teal" />
                <span className="text-slate-400">Views</span>
              </div>
              <div className="flex items-center space-x-2">
                <div className="w-3 h-3 rounded-full bg-pcc-gold" />
              ...
              <AreaChart data={trafficData}>
                <defs>
                  <linearGradient id="colorViews" x1="0" y1="0" x2="0" y2="1">
                    <stop offset="5%" stopColor="#2d5a47" stopOpacity={0.3} />
                    <stop offset="95%" stopColor="#2d5a47" stopOpacity={0} />
                  </linearGradient>
                ...
                />
                <Area type="monotone" dataKey="views" stroke="#2d5a47" fillOpacity={1} fill="url(#colorViews)" />
                <Area type="monotone" dataKey="visitors" stroke="#c9a227" fillOpacity={1} fill="url(#colorVisitors)" />
              </AreaChart>
            </ResponsiveContainer>
          </div>
        </div>

        {/* Traffic Sources - PCC brand colors */}
          ...
          <div className="space-y-4">
            {topArticles.map((article, i) => (
              <div key={i} className="flex items-center space-x-4">
                <div className="w-8 h-8 rounded-lg bg-pcc-teal/10 flex items-center justify-center text-sm font-bold text-pcc-teal">
                  {i + 1}
                </div>
          ...
          <div className="h-64">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={contentPerformance} layout="vertical">
                ...
                />
                <Bar dataKey="views" fill="#2d5a47" radius={[0, 4, 4, 0]} />
              </BarChart>
            </ResponsiveContainer>
          </div>
        </div>
